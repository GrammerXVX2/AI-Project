...
from backend.app.schemas import UpdateMessageRequest, UserRequest
from backend.app.config import DATA_DIR, SUMMARY_CHECKPOINT_SIZE, SUMMARY_MAX_WORDS, STATIC_DIR
from backend.app.services.history_manager import history_manager
...
def _get_mermaid_doc_filename(m_type: str) -> Optional[str]:
    mapping = {
        "graph TD": "flowchart.md",
        "flowchart": "flowchart.md",
        "sequenceDiagram": "sequenceDiagram.md",
        "classDiagram": "classDiagram.md",
        "stateDiagram": "stateDiagram.md",
        "erDiagram": "entityRelationshipDiagram.md",
        "gantt": "gantt.md",
        "journey": "userJourney.md",
        "gitGraph": "gitgraph.md",
    }
    return mapping.get(m_type)
...
async def process_chat(request: UserRequest):
    ...
    if _needs_mermaid_guardrail(user_query):
        ...
        doc_content = ""
        doc_file = _get_mermaid_doc_filename(mermaid_type)
        if doc_file:
            try:
                # Assuming mermaid-docs is inside static/mermaid-docs
                doc_path = STATIC_DIR / "mermaid-docs" / doc_file
                if doc_path.exists():
                    doc_content = doc_path.read_text(encoding="utf-8")
                    # Limit content length to avoid overflowing context too much if doc is huge
                    if len(doc_content) > 4000:
                         doc_content = doc_content[:4000] + "\n...(truncated)"
            except Exception as e:
                print(f"Failed to read mermaid doc {doc_file}: {e}")
        ...
        if doc_content:
            mermaid_hint += f"\n\n[REFERENCE SYTAX FOR {mermaid_type}]:\n{doc_content}\n[END REFERENCE]"
    ...
    if request.system_prompt and request.system_prompt.strip():
        ...
        if decision == "CODING":
            ...
            system_msg = (
                "You are an expert coding assistant and knowledge base expert.\n"
                "IMPORTANT: When writing code, ALWAYS wrap it in Markdown code blocks specifying the language.\n"
                "Example:\n"
                "```python\n"
                "print('Hello')\n"
                "```\n"
                "Never write code without the language identifier (e.g., use ```csharp for C#, ```javascript for JS)."
            )
    ...
    async def summarize_chat(history: list, prev_summary: Optional[str]):
        ...
        def _is_parrot(summary: str, context: str) -> bool:
            norm_sum = _normalize(summary)
            norm_ctx = _normalize(context)
            if not norm_sum or not norm_ctx:
                return False
            # Direct containment or high token overlap signals parroting.
            if norm_sum in norm_ctx:
                return True
            sum_tokens = norm_sum.split()
            if not sum_tokens:
                return False
            ctx_tokens = set(norm_ctx.split())
            overlap = sum(1 for tok in sum_tokens if tok in ctx_tokens)
            return overlap / len(sum_tokens) >= 0.6
        ...
        sys = (
            "Сделай краткую выжимку диалога на русском. Без кода и без markdown. "
            "Не повторяй дословно сообщения и не перечисляй примеры кода — только суть. "
            "Не упоминай участников как 'первый/второй/третий собеседник'. "
            "1–2 предложения, до 30 слов. Можно указать язык, если понятен (например 'Язык: C#')."
        )
        usr = (
        ...
        try:
            ...
            # Try JSON if the model produced it, otherwise treat as plain text summary
            ...
            # Parroting guards
            if _is_parrot(summary, messages_text):
                return _fallback_from_history(history)
