...
SOURCE_DIRECTORY = str(BASE_DIR / "RAG data")

# Какие файлы читать (добавьте нужные расширения)
ALLOWED_EXTENSIONS = {'.py', '.js', '.ts', '.html', '.css', '.md', '.txt', '.json','.rs'}

# Игнорируемые папки
IGNORE_DIRS = {'.git', '__pycache__', 'node_modules', 'venv', 'env', '.idea', '.vscode', 'client', 'models', 'rag_db', 'Include', 'Lib', 'Scripts', 'share'}
        ...
        for file in files:
            ...
            if ext in ALLOWED_EXTENSIONS:
                ...
                try:
                    ...
                    # Добавляем метаданные, чтобы нейросеть знала, откуда код
                    docs.append({
                        "text": text,
                        "source": file_path,
                        "filename": file
